...
class CanGetAllInterns(EndpointPolicy):
    permissions = {Permission.intern_get_all}
    ...
    @staticmethod
    def validate_mentor_id(employee: MentorProfile, filters: InternListFilters) -> None:
        if employee.role in {UserRole.superuser, UserRole.head_mentor}:
            return
        ...
        if filters.mentor_id is not None and filters.mentor_id != employee.id:
            raise HTTPException(
                status_code=HTTP_403_FORBIDDEN,
                detail="You can only view your own interns",
            )
