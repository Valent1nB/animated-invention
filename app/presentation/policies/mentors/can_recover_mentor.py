...
class CanRecoverMentor(EndpointPolicy):
    permissions = {Permission.mentor_recover}
    ...
    async def check(employee: MentorProfile, mentor_id: UUID, uow: IUnitOfWork) -> None:
        if employee.id == mentor_id:
            raise HTTPException(status_code=HTTP_400_BAD_REQUEST, detail="Cannot recover self")
